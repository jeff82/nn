...
#def rbf(input,centre,width,Fun='Guassain')#TODO finish  rbf output

def rbf(x,centre,width):
    return exp(-width * norm(centre-x)**2)
...
class MuiltilayerANN(object):
    ...
    def __init__(self,NumofHiddenLayers,NumofNodesinHiddenlayers,inputDimension,outputDimension=1,maxIter=50,RbfCent=[],RbfWidth=[]):
    ...
    def printWeightMatrix(self):
        for idx in range(0,int(self.Nl)-1):
            ...
            print self.B[idx]
        return 0
    def forwardPropogation(self,singleDataInput,currentDataIdx,outfinc='RBF'):
    ...
    def rbfTeta(self,Anl,Y,Znl):#TODO finish rbf output err
        thetal=rbf(Anl,self.RbfCent,self.RbfWidth)-Y
        return  thetal


    
    def backwardPropogation(self,singleDataInput,currentDataIdx,outfunc='RBF'):
        ...
        thetaNl=self.calThetaNl(Atemp[self.Nl-2], outlabels, Ztemp[self.Nl-2])# need to change this
